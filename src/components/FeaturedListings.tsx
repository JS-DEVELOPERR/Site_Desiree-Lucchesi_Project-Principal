...
export default function FeaturedListings() {
  ...
  const nextSlide = () => {
    setCurrentSlide(current => 
      current + itemsPerSlide >= listings.length ? 0 : current + itemsPerSlide
    );
  };

  const prevSlide = () => {
    setCurrentSlide(current => 
      current - itemsPerSlide < 0 ? Math.max(0, listings.length - itemsPerSlide) : current - itemsPerSlide
    );
  };

  const visibleListings = listings.slice(currentSlide, currentSlide + itemsPerSlide);
          ...
          <div className="mt-20 relative">
            {listings.length > itemsPerSlide && (
              <>
                <button
                  onClick={prevSlide}
                  className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-12 bg-white dark:bg-gray-800 p-2 rounded-full shadow-lg hover:bg-gray-100 dark:hover:bg-gray-700 z-10"
                >
                  <ChevronLeft className="w-6 h-6 text-gray-600 dark:text-gray-300" />
                </button>
                <button
                  onClick={nextSlide}
                  className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-12 bg-white dark:bg-gray-800 p-2 rounded-full shadow-lg hover:bg-gray-100 dark:hover:bg-gray-700 z-10"
                >
